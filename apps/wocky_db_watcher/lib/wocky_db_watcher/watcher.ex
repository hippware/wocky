defmodule WockyDBWatcher.Watcher do
  ...
  def init([object, action]) do
    ...
    {:producer_consumer,
     %State{ref: ref, channel: channel, object: object, action: action}}
  end

  def handle_info(
        {:notification, _, ref, channel, payload},
        %State{ref: ref, channel: channel, action: :insert, object: object} =
          state
      ) do
    j = parse(payload)
  ...
  def handle_info(
        {:notification, _, ref, channel, payload},
        %State{ref: ref, channel: channel, action: :update, object: object} =
          state
      ) do
    j = parse(payload)
  ...
  def handle_info(
        {:notification, _, ref, channel, payload},
        %State{ref: ref, channel: channel, action: :delete, object: object} =
          state
      ) do
    j = parse(payload)
