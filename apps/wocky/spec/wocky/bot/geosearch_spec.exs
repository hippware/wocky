defmodule Wocky.Bot.GeosearchSpec do
  ...
  describe "user_distance_query/4" do
    before do
      ...
      {:ok, bots: visible_bots, middle_id: Enum.at(visible_bots, 2).id}
    end

    context "Queried user is owner" do
      before do
        query =
          &Geosearch.user_distance_query(
            shared.lat,
            shared.lon,
            shared.user.id,
            shared.user.id,
      ...
      it "should work for id lookups" do
        {results, rsm_out} =
          shared.query.(rsm_in(id: shared.middle_id, direction: :aft))
      ...
      it "should work in reverse" do
        {results, rsm_out} =
          shared.query.(rsm_in(id: shared.middle_id, direction: :before))
  ...
  describe "subscribed_distance_query/4" do
    before do
      ...
      query =
        &Geosearch.subscribed_distance_query(
          shared.lat,
          shared.lon,
          shared.user.id,
          &1
      ...
      {:ok, query: query, bots: bots, middle_id: Enum.at(bots, 2).id}
    end
    ...
    it "should limit as requested" do
      {results, rsm_out} =
        shared.query.(rsm_in(id: shared.middle_id, direction: :before, max: 2))
  ...
  describe "explore_nearby/5" do
    ...
    context "lots of bots" do
      before do
        ...
        subscribed_bots =
          Factory.insert_list(5, :bot, user: Factory.insert(:user))
        ...
        searchable_bots =
          Geosearch.get_all(shared.lat, shared.lon, user.id, false)
        ...
        Enum.each(other_bots, &Factory.insert(:share, user: user, bot: &1))

        {:ok, bots: searchable_bots}
      end

      it "should have all the searchable bots (circle limit)" do
        Geosearch.explore_nearby(
          GeoUtils.point(shared.lat, shared.lon),
          1_000_000_000.0,
          shared.user,
          100,
