defmodule RosterHelper do
  ...
  def make_friends(user1, user2) do
    Factory.insert(:roster_item, user_id: user1.id,
                   contact_id: user2.id)
    Factory.insert(:roster_item, user_id: user2.id,
                   contact_id: user1.id)
  end
  ...
  def follow(follower, followee) do
    Factory.insert(:roster_item, user_id: follower.id,
                   contact_id: followee.id, subscription: :to)
    Factory.insert(:roster_item, user_id: followee.id,
                   contact_id: follower.id, subscription: :from)
  end
