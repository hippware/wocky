defmodule Wocky.Callbacks.Relationship do
  ...
  defp check_share_notification(new_type, old_type, :friend, :friend, rec),
    do: handle_share_notification(new_type, old_type, rec, false)
  ...
  defp check_share_notification(
       ...
       do: handle_share_notification(new_type, :disabled, rec, true)
  ...
  defp check_share_notification(
       ...
       do: handle_share_notification(:disabled, old_type, rec, false)
  ...
  defp handle_share_notification(t, t, _rec, _new_friend?), do: :ok

  # Always share mode has been newly enabled from either disabled or nearby mode
  defp handle_share_notification(:always, _, rec, new_friend?),
    do: notify_share_start(rec, new_friend?)
  ...
  defp handle_share_notification(:nearby, _, _rec, _new_friend?), do: :ok

  # Share has been disabled
  defp handle_share_notification(:disabled, _, rec, _new_friend?),
    do: notify_share_end(rec)

  defp notify_share_start(share, new_friend?) do
    Hydrator.with_assocs(share, [:user, :contact], fn rec ->
      %LocationShare{
        ...
        other_user_share_type:
          Contacts.cached_share_type(rec.contact, rec.user),
