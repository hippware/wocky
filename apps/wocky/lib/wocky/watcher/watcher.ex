  ...
  defmacro __using__(opts) do
    quote do
      ...
      def handle_insert(event) do
        raise UndefinedFunctionError, "#{inspect __MODULE__} handle_insert/1 not defined"
      end

      def handle_update(event) do
        raise UndefinedFunctionError, "#{inspect __MODULE__} handle_update/1 not defined"
      end

      def handle_delete(event) do
        raise UndefinedFunctionError, "#{inspect __MODULE__} handle_delete/1 not defined"
      end
