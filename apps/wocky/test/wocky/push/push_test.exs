defmodule Wocky.PushTest do
  ...
  setup do
    ...
    {:ok, user: user, resource: user.resource, token: token}
  end
  ...
  describe "enable/4" do
    ...
    test "should disable old tokens", %{
      ...
      old_row =
        Repo.one(
          from Token,
            where: [user_id: ^user.id, resource: ^resource, valid: false]
        )
      ...
      new_row =
        Repo.one(
          from Token,
            where: [user_id: ^user.id, resource: ^resource, valid: true]
        )
  ...
  describe "notify/3" do
    ...
    test "should truncate long messages", shared do
      ...
      :ok = Push.notify(shared.user, shared.resource, sent_message)
    ...
    test "should create a db log entry", shared do
      ...
      log =
        Repo.one(
          from Log,
            where: [user_id: ^shared.user.id, resource: ^shared.resource]
        )
    ...
    test "token is invalidated when the service returns an error", shared do
      assert capture_log(fn ->
               :ok = Push.notify(shared.user, shared.resource, "bad token")
    ...
    test "should log an error when the push request fails and times out",
      ...
      assert capture_log(fn ->
               assert_raise RuntimeError, "requested_raise", fn ->
                 Push.notify(shared.user, shared.resource, "raise")
               end
             end) == ""
  ...
  describe "retry success" do
    test "should retry and succeed even if the first attempt fails", shared do
      ...
      assert fn ->
               Push.notify(shared.user, shared.resource, "retry test")
             end
             |> capture_log()
