...
test_iq_get_results() ->
  { "handle_iq with type get", setup, fun before_each/0, fun after_each/1,
    fun (Els) -> [
...
test_iq_set() ->
  ...
  { "handle_iq returns a error IQ when the IQ type is set", [
      ?_assertMatch(ErrorIQ, handle_iq(?JID, ?JID, #iq{type = set}))
  ]}.
