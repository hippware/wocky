...
test_is_participant() ->
    { "is_participant", [
      { "Returns true when a member of the specified chat is supplied", [
        ?_assert(is_participant(?ALICE_JID, ?GROUP_CHAT)),
        ?_assert(is_participant(?KAREN_JID, ?GROUP_CHAT))
      ]},
      { "Returns false for non- and removed members", [
        ?_assertNot(is_participant(?BOB_JID, ?GROUP_CHAT)),
        ?_assertNot(is_participant(?CAROL_JID, ?GROUP_CHAT))
      ]},
      { "Returns false for non-existant groups", [
        ?_assertNot(is_participant(?ALICE_JID, wocky_db:create_id()))
      ]}
    ]}.
