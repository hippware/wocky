...
%% @doc Returns `true' if the file ID is a valid Francus file ID
-spec is_valid_id(binary()) -> boolean().
is_valid_id(FileID) ->
    try
        ossp_uuid:import(FileID, binary),
        true
    catch
        _:_ ->
            false
...
-spec keep(wocky_db:context(), file_id()) -> ok | not_found.
keep(Context, FileID) ->
    FileCond = #{id => FileID},
